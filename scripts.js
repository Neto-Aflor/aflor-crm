document.addEventListener("DOMContentLoaded", function () {
  ...
  function kpis(){
    ...
    document.getElementById('kBadge1').textContent=leads.filter(l=>l.status).length+' com situação';
  ...
  function om(l){
    ...
    const diagChip=l.diagnostico&&l.diagnostico.toLowerCase().startsWith('s')
      ? '<span class="chip cDiag">✓ Diagnóstico</span>'
      : '<span class="chip cDiagNo">✕ Diagnóstico</span>';
    const acessoChip=l.acesso?`<a class="chip cS m-access-chip" href="${x(l.acesso)}" target="_blank">Acesso Rápido</a>`:'';
